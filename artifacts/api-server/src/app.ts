...
export function createApp(deps: {
  ...
  const allowedOrigins: Array<string | RegExp> = [
    ...
    // Railway services and still pass CORS. This was documented in
    // RAILWAY_DEPLOY.md as already-implemented, but the pattern was never
    // actually added here — every cross-origin request from a Railway
    // frontend domain was being rejected, surfacing to the browser as a
    // generic "TypeError: Failed to fetch" (CORS failures don't produce a
    // readable error — the browser blocks the response before JS sees it).
    /\.railway\.app$/,
    /localhost/,
